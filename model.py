...
class TimeEmbd(nn.Module):
    ...
    def forward(self, t):
        ...
        t = t.unsqueeze(0)
        #print(f'In T. Embd: {t.shape}')
        p1 = (omega * t).sin()
        p2 = (omega * t).cos()
        #print(f'In T. Embd: {p1.shape}')
...
class Diffusion(nn.Module):
    ...
    def noise(self, x):
        ...
        #mu = torch.prod(torch.tensor([self._mu(t) for t in self.tspan]))
        mu = self.alpha_bar()
        #var = torch.prod(torch.tensor([self._var(t) for t in self.tspan]))
    ...
    def forward(self, x, y):
        ...
        for t in torch.linspace(1, 0, self.steps)[:-1]:
            ...
            #print(f'Shape of target: {x.shape}')
    ...
    def sample(self, x, g):
        ...
        """
        ### Set all inputs to the correct shape
        ...
        ### Step by step up for the denoising
        ### Equivalent to self.forward without generation of noisy and error
        ### calculation
        for t in torch.linspace(1, 0, self.steps)[:-1]:
            pred = self.up(x_noisy, y, t)
            e = torch.randn_like(pred)
