...
class DiffusionModel(nn.Module):
    def __init__(
        self, beta_start=0.0001, beta_end=0.02, num_timesteps=1000, *args, **kwargs
    ):
        """
        ...
        self.alpha_cumprod_prev = torch.cat(
            [torch.tensor([1.0]), self.alpha_cumprod[:-1]], axis=0
        )
    ...
    def q_sample(self, x_start, t, noise):
        ...
        sqrt_alpha_cumprod = (
            torch.sqrt(self.alpha_cumprod[t]).view(batch_size, 1, 1).to(x_start.device)
        )
    ...
    def p_sample(self, model, x, t, t_index, genre):
        ...
        sqrt_one_minus_alpha_cumprod = torch.sqrt(1 - self.alpha_cumprod[t_index]).view(
            -1, 1, 1, 1
        )
        posterior_variance = (
            self.beta[t_index]
            * (1 - self.alpha_cumprod_prev[t_index])
            / (1 - self.alpha_cumprod[t_index])
        )

        # Calculate the predicted mean by the model
        model_mean = (
            x - sqrt_one_minus_alpha_cumprod * model(x, genre, t)
        ) / sqrt_alpha_cumprod
    ...
    def sample(self, model, shape, genre, timesteps=1000):
        ...
        for t_index in tqdm(
            ...
            desc=f"Generating sample with {timesteps} diffusion steps",
        ):
